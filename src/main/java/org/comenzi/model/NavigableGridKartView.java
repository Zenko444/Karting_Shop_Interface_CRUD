...
@Route(value = "karturi", layout = MainLayout.class)
public class NavigableGridKartView extends VerticalLayout {

    private Grid<Kart> grid = new Grid<>(Kart.class);
    private TextField searchField = new TextField();
    private List<Kart> allItems;

    public NavigableGridKartView() {
        setSizeFull();
        setAlignItems(Alignment.CENTER);
        getStyle().set("background", "transparent");

        Button btnAdd = new Button("Adaugă Kart");
        btnAdd.addThemeVariants(ButtonVariant.LUMO_PRIMARY);
        btnAdd.addClickListener(e -> creeazaSiNavigheaza());

        searchField.setPlaceholder("Caută ID sau Model...");
    ...
    private void configureGrid() {
        ...
        grid.addColumn(Kart::getId).setHeader("ID").setWidth("80px").setFlexGrow(0);
        grid.addColumn(k -> k.getProducator() != null ? k.getProducator().getNume() : "-").setHeader("Producător");
        grid.addColumn(Kart::getDenumire).setHeader("Model");
        grid.addColumn(Kart::getTipMotor).setHeader("Motor");
        grid.addColumn(Kart::getCategorie).setHeader("Categorie");
        grid.addColumn(k -> k.getPret() + " RON").setHeader("Preț");

        grid.addComponentColumn(item -> {
            Button btnEdit = new Button("Edit");
            btnEdit.addThemeVariants(ButtonVariant.LUMO_SMALL);
            btnEdit.addClickListener(e -> getUI().ifPresent(ui -> ui.navigate(FormKartView.class, item.getId())));
    ...
    private void fetchData() {
        ...
        EntityManager em = emf.createEntityManager();
        try {
            allItems = em.createQuery("SELECT k FROM Kart k", Kart.class).getResultList();
            filtreazaGrid();
        } finally {
            em.close();
            emf.close();
    ...
    private void filtreazaGrid() {
        ...
        if (allItems != null) {
            if (val == null || val.isEmpty()) {
                ...
                List<Kart> filtered = allItems.stream()
                        .filter(k -> String.valueOf(k.getId()).contains(val) || k.getDenumire().toLowerCase().contains(val.toLowerCase()))
    ...
    private void creeazaSiNavigheaza() {
        ...
        try {
            em.getTransaction().begin();
            Kart k = new Kart("Model Nou", 0.0, null, "-", "-");
            em.persist(k);
            em.getTransaction().commit();
            getUI().ifPresent(ui -> ui.navigate(FormKartView.class, k.getId()));
        } finally {
            em.close();
            emf.close();
        }
